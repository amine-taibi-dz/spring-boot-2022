		...
		if(auth != null && auth.startsWith(PREFIX)) {
			...
			if(userName!=null && SecurityContextHolder.getContext().getAuthentication() ==null) {
				...
				if(jwtUtils.validateToken(jwt, userDetails)) {
					UsernamePasswordAuthenticationToken usPswdAuthenticationToken = 
							new UsernamePasswordAuthenticationToken(userDetails,null,userDetails.getAuthorities());
					usPswdAuthenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
					SecurityContextHolder.getContext().setAuthentication(usPswdAuthenticationToken);
				}				
			}			
		}
